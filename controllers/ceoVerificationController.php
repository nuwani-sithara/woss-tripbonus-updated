...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Check if CEO has already verified or rejected this month/year
    $ceoCheckSql = "SELECT * FROM ceoverify WHERE month = ? AND year = ?";
    $ceoCheckStmt = $conn->prepare($ceoCheckSql);
    $ceoCheckStmt->bind_param("si", $month, $year);
    $ceoCheckStmt->execute();
    $ceoResult = $ceoCheckStmt->get_result();
    $ceoVerified = $ceoResult->num_rows > 0;
    $ceoData = $ceoResult->fetch_assoc();
    ...
    $approverSql = "SELECT pv.paymentVerifyBy, pv.paymentVerifyDate, u.fname, u.lname FROM paymentverify pv JOIN users u ON pv.paymentVerifyBy = u.userID WHERE pv.month = ? AND pv.year = ? LIMIT 1";
...
// CEO verifies or rejects all payments for a month/year
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ceoID = isset($_SESSION['userID']) ? $_SESSION['userID'] : null; // Use logged-in user
    ...
    $checkSql = "SELECT * FROM ceoverify WHERE month = ? AND year = ?";
    ...
    // Insert into ceoverify
    $sql = "INSERT INTO ceoverify (ceoVerifyBy, month, year, ceoVerifyDate, approval_status, comment) VALUES (?, ?, ?, CURDATE(), ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("isiis", $ceoID, $month, $year, $approval_status, $comment);
    $stmt->execute();
    $ceoVerifyID = $conn->insert_id;

    // Insert into approvals
    $approval_stage = 'ceo_verification';
    $approval_by = $ceoID;
    $approval_date = date('Y-m-d H:i:s');
    $sql2 = "INSERT INTO approvals (ceoVerifyID, approval_status, approval_stage, approval_by, approval_date) VALUES (?, ?, ?, ?, ?)";
    $stmt2 = $conn->prepare($sql2);
    $stmt2->bind_param("iisis", $ceoVerifyID, $approval_status, $approval_stage, $approval_by, $approval_date);
    $stmt2->execute();
    $approvalID = $conn->insert_id;

    // Insert into ceoapprover
    $sql3 = "INSERT INTO ceoapprover (approvalID) VALUES (?)";
    ...
    if ($approval_status === 1) {
        // CEO verified
        $notifyUrl = 'https://tripbonus.worldsubsea.lk/controllers/sendCeoVerifyNotificationController.php';
    } else {
        // CEO rejected
        $notifyUrl = 'https://tripbonus.worldsubsea.lk/controllers/sendCeoRejectNotificationController.php';
    }
    $notifyData = [
        'ceoVerifyID' => $ceoVerifyID,
