...
try {
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                <p>The payments for <strong>$month $year</strong> have been <strong>rejected</strong> by Accountant: <strong>$accName</strong> on $verifyDate.</p>
    ...
    $mail->Body = $body;
    file_put_contents(__DIR__ . '/mail_debug.log', "Before send\n", FILE_APPEND);
    try {
        $mail->send();
        file_put_contents(__DIR__ . '/mail_debug.log', "After send (success)\n", FILE_APPEND);
    } catch (Exception $e) {
        file_put_contents(__DIR__ . '/mail_debug.log', "Exception: " . $mail->ErrorInfo . "\n", FILE_APPEND);
        throw $e;
    }

    // Log notification
    $stmt = $conn->prepare("INSERT INTO payment_notifications (publishedID, accountantID, recipientID, recipient_role, notification_type, email_sent, sent_at, reason, status) VALUES (?, ?, ?, 'operation_manager', 'reject', 1, NOW(), ?, 1)");
    $stmt->bind_param("iiis", $data['publishedID'], $accountantID, $om['userID'], $reason);
    $stmt->execute();
